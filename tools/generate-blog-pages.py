# -*- coding: utf-8 -*-
"""
ShikarpuriAchar.pk — Blog page + sitemap generator
------------------------------------------------------
Reads blog.json (the single source of truth for articles) and generates:
  1. /blog/<slug>.html for EVERY article — a thin dynamic shell with the
     article's SEO meta (title, description, keywords, Open Graph,
     canonical, Article + BreadcrumbList JSON-LD) baked in for crawlers.
     All visible content is still rendered client-side by main.js from
     blog.json, so nothing is duplicated.
  2. sitemap.xml covering all core pages + every product page + every
     blog article (reads products.json too, so re-running this script
     never drops the product URLs tools/generate-product-pages.py added).

Run from the project root:
    python tools/generate-blog-pages.py

Re-run whenever you add/remove an article in blog.json.
"""
import io
import json
import os
...
<meta property="og:image:alt" content="__TITLE__ — ShikarpuriAchar.pk">
  ...
  <div class="header-inner">
    <a href="/" class="logo" aria-label="ShikarpuriAchar.pk home">
      ...
      <span class="logo__text">
        ...
        <span class="logo__text-sub">Since 1974</span>
      </span>
    </a>
...
<main id="main">
  ...
  <section class="section section--tight">
    <div class="container" style="max-width:760px">
      <div data-blog-article-root>
        <div class="text-center" style="padding:3rem"><p>Loading article...</p></div>
      </div>
    </div>
  </section>
  ...
  <section class="section section--tight section--alt">
    <div class="container">
      <div class="related-heading text-center" style="margin-top:0" data-reveal>
        ...
        <h2 style="font-size:clamp(1.4rem,3vw,1.9rem)">You Might Also Like</h2>
      </div>
      <div class="grid grid-3" data-blog-related-products></div>
    </div>
  </section>
</main>
  ...
  <div class="container">
    <div class="footer-top">
      <div class="footer-brand">
        ...
        <p>Original Shikarpuri Achar since 1974 — traditional Sindhi pickles made with premium ingredients, delivered across Pakistan.</p>
    ...
    <div class="footer-bottom">
      ...
      <span>Original Shikarpuri Achar Since 1974</span>
    </div>
  </div>
</footer>
...
def write_sitemap(products, articles):
...
def main():
    with io.open(os.path.join(ROOT, "blog.json"), encoding="utf-8") as f:
        articles = json.load(f)["articles"]
    products_path = os.path.join(ROOT, "products.json")
    products = []
    if os.path.exists(products_path):
        with io.open(products_path, encoding="utf-8") as f:
    ...
    print(f"Done. {len(articles)} blog article pages generated.")
